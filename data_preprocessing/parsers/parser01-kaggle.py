...
if __name__ == "__main__":
	with open('Team_Attributes.csv') as team_attributes_file, open('Match.csv') as match_file, open('learning_vectors.csv', 'w+') as output:
		fieldnames = ['Match_id', 'League_id', 'Season', 'Stage', 'Date' ,'Result', 'B365H', 'B365D' ,'B365A' ,'HomeTeam_buildUpPlaySpeed' ,'HomeTeam_buildUpPlayPassing', 'HomeTeam_chanceCreationShooting', 'HomeTeam_defencePressure', 'AwayTeam_buildUpPlaySpeed', 'AwayTeam_buildUpPlayPassing', 'AwayTeam_chanceCreationShooting', 'AwayTeam_defencePressure']
		...
		for match_row in match_reader:
			...
			writer.writerow({	'Match_id': match_row['id'], 'League_id' : match_row['league_id'],'Season': match_row['season'], 'Stage': match_row['stage'], 'Date': match_date, 'Result': result, 'B365H': match_row['B365H'], 'B365D': match_row['B365D'] ,'B365A': match_row['B365A'],
